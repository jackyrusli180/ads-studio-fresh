...
def edit_image(request):
    ...
    try:
        ...
        try:
            # Import the correct module from google-genai
            from google import genai
            
            # Create the client as per new SDK documentation
            ...
            # Prepare the content in the format expected by the new SDK
            contents = [
                text_input,
                {
                    "inline_data": {
            ...
            # Make the API call using the new SDK format
            ...
            # Check for response.parts directly (new SDK structure)
            if hasattr(response, 'parts'):
                for part in response.parts:
                    ...
                    if hasattr(part, 'text') and part.text:
                        ...
                        logger.info(f"Received text response from Gemini: {part.text[:100]}...")
                    
                    # Handle image parts
                    if hasattr(part, 'inline_data') and part.inline_data:
                        try:
                            # Save the generated image
                            logger.info("Received image response from Gemini")
                            ...
                            if original_image_id:
                                try:
                                    original_image = AIGeneratedImage.objects.get(id=original_image_id)
                                    edited_image = AIGeneratedImage.objects.create(
                                        user=request.user,
                                        prompt=f"[Edited] {original_image.prompt} - {prompt}",
                                        image_url=image_url,
                                        resolution=original_image.resolution,
                            ...
                            # Convert the binary data to base64 for the frontend
                            ...
                            logger.exception(f"Error processing image data: {str(img_error)}")
            
            # Try the candidates approach as fallback (old SDK structure)
            ...
            # If no useful parts found in the response
            if not result['parts']:
                logger.error("No content parts in the response")
                return Response({
                    'error': 'No content generated by the image editing service'
                }, status=status.HTTP_502_BAD_GATEWAY)
            ...
            if "is not found for API version" in str(api_error):
                return Response({
